        ...
        <div class="panel-body">
            ...
            <table id="table" class="table table-bordered table-striped table-hover table-datatable">
                ...
                <tbody>
                    ...
                    include '../koneksi.php';
                    $no = 1;
                    $arsip = mysqli_query($koneksi, "SELECT * FROM arsip,kategori,petugas WHERE arsip_petugas=petugas_id and arsip_kategori=kategori_id ORDER BY arsip_id ASC");
                            ...
                            <td class="text-center">
                                ...
                                <div class="btn-group">
                                    ...
                                    <a target="_blank" class="btn btn-default"
                                        href="riwayat_arsip_download.php?id=<?php echo $p['arsip_id']; ?>"><i
                                            class="fa fa-download"></i></a>

                                    <a target="_blank" href="arsip_preview.php?id=<?php echo $p['arsip_id']; ?>"
                                        class="btn btn-default"><i class="fa fa-search"></i> Preview</a>

                                    <button disabled style="display: none;" type="button" class="btn btn-primary"
                                        data-toggle="modal" data-target="#exampleModal_<?php echo $p['arsip_id']; ?>">
                                    ...
                                    <button type="button" class="btn btn-primary tombol_hapus"
                                        data-arsip-id="<?php echo $p['arsip_id']; ?>">
                                        <i class="fa fa-trash"></i>
                                    </button>
                                </div>
                            </td>
...
<script>
    const tombol_hapus = document.querySelectorAll('.tombol_hapus');

    tombol_hapus.forEach(button => {
        button.addEventListener('click', function () {
            const konfirmasi = confirm("Apakah Anda yakin ingin menghapus data ini?");

            if (konfirmasi) {
                const arsipId = this.dataset.arsipId;
                window.location.href = `arsip_hapus.php?id=${arsipId}`;
            } else {
