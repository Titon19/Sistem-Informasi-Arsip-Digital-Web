...
if (!empty($filter_ids)) {
    ...
    $sql = "$sql_admin WHERE riwayat_preview_id IN ($filter_ids)
            UNION
            $sql_user WHERE riwayat_preview_id IN ($filter_ids)
            UNION
            $sql_petugas WHERE riwayat_preview_id IN ($filter_ids)
            ORDER BY riwayat_preview_id DESC";
...
$tahun = $row ? date('Y', strtotime($row['riwayat_preview_waktu'])) : date('Y');

require_once __DIR__ . '/../vendor/autoload.php';

$html = '
<div style="text-align:center">
    <h1>RIWAYAT ARSIP PREVIEW</h1><br>
    <h1>Tahun ' . $tahun . '</h1>
</div>
...
do {
    ...
    <tr>
        <td>' . $no++ . '</td>
        <td>' . date('H:i:s  d-m-Y', strtotime($row['riwayat_preview_waktu'])) . '</td>
        <td>' . $row['nama_user'] . '</td>
